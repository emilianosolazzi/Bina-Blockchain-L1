...
const TEMPORAL_SEED_LENGTH: usize = 8;
const TEMPORAL_SEED_BOM: u8 = 0x00;
...
fn encode_temporal_seed(timestamp_secs: u64) -> Result<[u8; TEMPORAL_SEED_LENGTH]> {
    if timestamp_secs > MAX_TEMPORAL_SEED_TIMESTAMP {
        return Err(anyhow!(
            "timestamp {} exceeds 56-bit temporal seed capacity",
            timestamp_secs
        ));
    }
...
fn decode_temporal_seed_timestamp(temporal_seed: &[u8]) -> Result<u64> {
    ...
    if temporal_seed[0] != TEMPORAL_SEED_BOM {
        return Err(anyhow!(
            "invalid temporal seed BOM marker: expected 0x{TEMPORAL_SEED_BOM:02x}, got 0x{:02x}",
            temporal_seed[0]
        ));
    }
...
fn generate_temporal_seed() -> Result<Vec<u8>> {
    let now_secs = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs();

    Ok(encode_temporal_seed(now_secs)?.to_vec())
}
...
mod tests {
    ...
    #[test]
    fn test_temporal_seed_matches_contract_format() {
        let timestamp = 1_705_000_000u64;
        let temporal_seed = encode_temporal_seed(timestamp).unwrap();

        assert_eq!(temporal_seed.len(), TEMPORAL_SEED_LENGTH);
        assert_eq!(temporal_seed[0], TEMPORAL_SEED_BOM);
        assert_eq!(&temporal_seed[1..], &timestamp.to_be_bytes()[1..]);
    }

    #[test]
    fn test_temporal_seed_round_trips_timestamp() {
        let timestamp = 1_746_385_920u64;
        let temporal_seed = encode_temporal_seed(timestamp).unwrap();

        assert_eq!(decode_temporal_seed_timestamp(&temporal_seed).unwrap(), timestamp);
    }

    #[test]
    fn test_temporal_seed_rejects_invalid_bom() {
        let mut temporal_seed = encode_temporal_seed(1_705_000_000u64).unwrap();
        temporal_seed[0] = 0x01;

        assert!(decode_temporal_seed_timestamp(&temporal_seed).is_err());
    }
